...
async def _email_job(app) -> None:
    """Send daily news digest email at 10:05 KST."""
    from email_digest import send_digest
    import sheets as _sheets
    log.info("[email] Starting daily digest job...")
    try:
        ...
        if result:
            log.info("[email] Daily digest sent successfully.")
        else:
            log.warning("[email] Daily digest failed to send.")
    except Exception as e:
        log.error("[email] Digest job error: %s", e)
